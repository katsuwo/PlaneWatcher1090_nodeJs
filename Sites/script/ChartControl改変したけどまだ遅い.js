    ...
    var AJ_comInterval = 3000;
    var CH_aircrafts = new Array();
...
*/
    ...
    function CH_chartAddRow(acData) {
        ...
        if (newRows.length != 0) {
            var newData = CH_generateData(newRows);
            var allRows = CH_waTable.getData(false);
            Array.prototype.push.apply(allRows.rows,newData.rows);
...
//		var count = 0;
        ...
        var renderedData = CH_waTable.getData(false,false,true);
        if (renderedData == undefined) {
            return;
        }

        var renderedRows = renderedData.rows;
        var update = false;
        var updateRow = new Array();

		for (var count in renderedRows){
            var renderedICAO = renderedRows[count].icao;
            if (acData[renderedICAO] == undefined){
                //tihs aircraft is not available update data. 
                continue;
            }

            update = true;
            var row = CH_waTable.getRow(renderedICAO).row;
            var tmpAC = acData[renderedICAO];
            row.icao = tmpAC.ADDR;
            row.flight = tmpAC.FLIGHT;
            row.squak = tmpAC.SQUAWK;
        ...
        Platform.performMicrotaskCheckpoint();
        if (update == true) {
//            CH_waTable.setData(renderedData);
        }
	}
